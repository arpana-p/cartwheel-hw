...
def list_my_orders(ctx: AuthContext) -> dict[str, Any]:
    ...
    if ctx.role == "support":
        return {
            "ok": False,
            "error": "invalid_argument",
            "reason": "support users have no orders of their own; use get_order instead",
        }
...
def find_order(ctx: AuthContext, query: str) -> dict[str, Any]:
    ...
    try:
        if ctx.role == "shopper":
            ...
            user_ids = [row[0] for row in conn.execute("SELECT DISTINCT user_id FROM orders")]
            orders = [
                order
                for user_id in user_ids
                for order in db.list_orders_for_user(conn, user_id, limit=10_000)
            ]
        ...
        for order in orders:
            ...
            if product is None:
                continue
            score = max(
                fuzz.partial_ratio(query.lower(), product.title.lower()),
                fuzz.token_set_ratio(query.lower(), product.title.lower()),
            )
            if score < 60:
                continue
            ranked_matches.append((score, order, product.title))

        ranked_matches.sort(key=lambda match: (-match[0], -match[1].id))
        matches = []
        for _, order, product_name in ranked_matches[:5]:
            matches.append({"id": order.id, "product_name": product_name, **order.to_public_dict()})
